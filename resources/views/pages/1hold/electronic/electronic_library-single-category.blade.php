...
@section('title')
<title>{{ $electronic_library_category->home_title_ar}}</title>
@endsection

@section('meta')
    <meta name="description" content="{{$electronic_library_category->description_ar}}">
    <meta name="keywords" content="{{$electronic_library_category->keywords_ar}}">

         <meta property="og:title"  content="{{$electronic_library_category->home_title_ar}}"/>
         <meta property="title"  content="{{$electronic_library_category->home_title_ar}}"/>


    <meta property="og:description" content="{{$electronic_library_category->description_ar}}"/>
    <meta property="og:url"       content="{{asset('/online-collection/'.$electronic_library_category->url_ar)}}"/>
...
<div class="pageContent">
	...
	<div class="breadcrumbs">
	    <div class="container">
	        ...
	        <i class="fa fa-long-arrow-left"></i><span>   المكتبة الالكترونية</span>
	        <i class="fa fa-long-arrow-left"></i><span>{{$electronic_library_category->home_title_ar}}</span>
	    </div>
	</div>
	<!------------------------------------------------------------------------------------------------------------------------------------>
					...
					<div class="sidebar-widgets">
						<ul>
							<li class="widget w-recent-posts">
								<h4 class="widget-title"><span class="main-color">احدث   {{$electronic_library_category->home_title_ar}}</span></h4>
								<div class="widget-content">
									<ul>
										@foreach($electronic_library_latest as $val)
										<?php 
										...
										<li>
											<div class="post-img">
												<a href="{{asset('/single-online-book/'.$val->url_ar)}}"><img src="{{asset($val->image)}}" alt="{{$val->image_alt_ar}}"></a>
											</div>
											<div class="widget-post-info">
												<h5><a href="{{asset('/single-online-book/'.$val->url_ar)}}">{{$val->title_ar}}</a></h5>
												<div class="meta">
							...
							<li class="widget  w-recent-posts">
								<h4 class="widget-title"><span class="main-color">اهم  {{$electronic_library_category->home_title_ar}}</span></h4>
								<div class="widget-content">
									<ul>
										@foreach($electronic_library_random as $val)
										<?php 
										    $months = array("Jan" => "يناير", "Feb" => "فبراير", "Mar" => "مارس", "Apr" => "أبريل", "May" => "مايو", "Jun" => "يونيو", "Jul" => "يوليو", "Aug" => "أغسطس", "Sep" => "سبتمبر", "Oct" => "أكتوبر", "Nov" => "نوفمبر", "Dec" => "ديسمبر");
										    $your_date = $val->created_at->format('y-m-d'); // The Current Date
										    $en_month = $val->created_at->format("M", strtotime($your_date)) ;
							...
							<div class="portfolio grid p-2-cols p-style3" id="grid">
								@foreach($electronic_library_order as $val)
									...
									<div class="portfolio-item">
										<figure>
											<img height="220px" src="{{asset($val->image)}}" alt="{{$val->image_alt_ar}}">
											<figcaption>
												<div class="port-captions">
													<h4><a href="{{asset('/single-online-book/'.$val->url_ar)}}">  {{$val->title_ar}}</a></h4>
												</div>
												<div class="icon-links">
													<a href="{{asset('/single-online-book/'.$val->url_ar)}}" class="link"><i class="fa fa-link"></i></a>
