...
<div class="send-message body-bg">
 	<div class="container">
 		...
 		<div class="row messages">
            ...
            ->where('mark',0)
            ->where('poduct_buy_id',$poduct_buy->id)
 		...
 		<div class="col-md-9">
 			<div class="message-box" style="    overflow-y: scroll;">
                    ...
                    @foreach($single_chat as $val)
                        ...
                        @if($val->seller_id == Auth::guard('seller')->user()->id && $val->message_seller != null)
                        <div class="recive" style="padding:5px;">
                            <p><img src="{{asset(Auth::guard('seller')->user()->avatar)}}" alt="chat" style="width: 33px;height: 33px;border-radius: 50%;display: inline;margin-left: 5px;"> {!! $val->message_seller !!}</p>
                        </div>
                        @endif

                        @if($val->buyer_id == $single_buyer->id && $val->message_buyer != null)
                        <div class="replay" style="background: #dad8d8;margin-top:20px;padding:5px;direction: ltr;">
                            <p><img src="{{asset( $single_buyer->avatar)}}" alt="chat" style="width: 33px;height: 33px;border-radius: 50%;display: inline;margin-left: 5px;"> {!! $val->message_buyer !!}.</p>
                        </div>
                        @endif
            ...
            <form id="chat_form" method="post" action="{{ asset('chat_form') }}" enctype="multipart/form-data" >                 
                ...
                <div class="write-message" style="margin-top:20px;border:1px solid #2da0d6;padding:5px;">
                    <input type="" name="buyer_id" value="{{$single_buyer->id}}" hidden="">
                    <input type="" name="seller_id" value="{{Auth::guard('seller')->user()->id}}" hidden="">
                    <input type="" name="poduct_buy_id" value="{{$poduct_buy->id}}" hidden="">
                     <textarea rows="1"  name="message_seller" style="width:100%;border:none"></textarea>
                </div>
