...
@section('title')
<title>{{ $visual_library_category->page_title_ar}}</title>
@endsection

@section('meta')
    <meta name="description" content="{{$visual_library_category->description_ar}}">
    <meta name="keywords" content="{{$visual_library_category->keywords_ar}}">

         <meta property="og:title"  content="{{$visual_library_category->home_title_ar}}"/>
         <meta property="title"  content="{{$visual_library_category->home_title_ar}}"/>


    <meta property="og:description" content="{{$visual_library_category->description_ar}}"/>
    <meta property="og:url"       content="{{asset('/visual-collection/'.$visual_library_category->url_ar)}}"/>
...
<div class="pageContent">
	...
	<div class="breadcrumbs">
	    <div class="container">
	        ...
	        <i class="fa fa-long-arrow-left"></i><span>   المكتبة  المرئية</span>
	        <i class="fa fa-long-arrow-left"></i><span>{{$visual_library_category->home_title_ar}}</span>
	    </div>
	</div>
	<!------------------------------------------------------------------------------------------------------------------------------------>
					...
					<div class="sidebar-widgets">
						<ul>
							<li class="widget w-recent-posts">
								<h4 class="widget-title"><span class="main-color">احدث   {{$visual_library_category->home_title_ar}}</span></h4>
								<div class="widget-content">
									<ul>
										@foreach($visual_library_latest as $val)
										<?php 
							...
							<li class="widget  w-recent-posts">
								<h4 class="widget-title"><span class="main-color">اهم  {{$visual_library_category->home_title_ar}}</span></h4>
								<div class="widget-content">
									<ul>
										@foreach($Visual_library_random as $val)
										<?php 
										    $months = array("Jan" => "يناير", "Feb" => "فبراير", "Mar" => "مارس", "Apr" => "أبريل", "May" => "مايو", "Jun" => "يونيو", "Jul" => "يوليو", "Aug" => "أغسطس", "Sep" => "سبتمبر", "Oct" => "أكتوبر", "Nov" => "نوفمبر", "Dec" => "ديسمبر");
										    $your_date = $val->created_at->format('y-m-d'); // The Current Date
										    $en_month = $val->created_at->format("M", strtotime($your_date)) ;
										...
										<figure>
											...
											<figcaption>
												<div class="port-captions">
													<h4><a href="{{asset('/single-visual-book/'.$val->url_ar)}}">  {{$val->title_ar}}</a></h4>
												</div>
												<div class="icon-links">
													<a href="{{asset('/single-visual-book/'.$val->url_ar)}}" class="link"><i class="fa fa-link"></i></a>
